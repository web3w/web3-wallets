...
class WalletConnectProvider implements IEthereumProvider {
  ...
  public async request<T = unknown>(args: RequestArguments): Promise<T> {
    ...
    if (signingMethods.includes(args.method)) {
      ...
      return new Promise(async (resolve, reject) => {
        this.signer.connection.on('payload', response => {
          // console.log('request.payload sign', response)
  ...
  public async sendAsync<T = unknown>(args: RequestArguments, callback): Promise<T> {
    // console.trace('sendAsync', args)
    if (args.method == 'eth_chainId') {
      const result = (this.signer.connection as any).chainId
      callback(undefined, { result })
      return Promise.resolve(result)
    }

    if (args.method == 'eth_accounts') {
      const result = (this.signer.connection as any).accounts
      callback(undefined, { result })
      return Promise.resolve(result)
    }

    if (signingMethods.includes(args.method)) {
      // this.signer.request(args)
      console.log('sendAsync.payload', args.method)
      return new Promise(async (resolve, reject) => {
        this.signer.connection.on('payload', response => {
          if (isJsonRpcError(response)) {
